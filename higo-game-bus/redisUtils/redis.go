...
func HSet(key string, field string, value string, survivalTime int) error {
...
// HSetAll 向名称为key的hash中添加元素field
func HSetAll(key string, value map[string]string, survivalTime int) error {
	res := Pool.HMSet(context.Background(), key, value)
	if res.Err() != nil {
		return res.Err()
	}
	if survivalTime > 0 {
...
func HGETALL(key string) (map[string]string, error) {
	return Pool.HGetAll(context.Background(), key).Result()
}
...
func XGroupCreate(key string, group string) error {
	res := Pool.XGroupCreate(context.Background(), key, group, "$")
	return res.Err()
}

func XGroupCreateConsumer(key string, group string, consumer string) error {
	res := Pool.XGroupCreateConsumer(context.Background(), key, group, consumer)
	return res.Err()
}

func XGroupDelConsumer(key string, group string, consumer string) error {
	res := Pool.XGroupDelConsumer(context.Background(), key, group, consumer)
	return res.Err()
}

//XReadGroup 从名称为key的stream中读取数据
func XReadGroup(key string, group string, consumer string) ([]byte, string, error) {
	...
	return []byte(res.Val()[0].Messages[0].Values["value"].(string)), res.Val()[0].Messages[0].ID, nil
}

// XAck 从名称为key的stream中删除数据
func XAck(key string, group string, id string) error {
	res := Pool.XAck(context.Background(), key, group, id)
	return res.Err()
}
