...
func (i *InfoResponse) Zip() string {
	d, _ := json.Marshal(i)
	return utils.ZipString(fmt.Sprintf("%s", d))
}

type MoveResponse struct {
	NextColor int `json:"next_color"` // 下一步颜色 0 任意 1 黑 2 白
}
...
type OwnerShipResponse struct {
	...
	AnalysisData AnalysisScoreData `json:"analysis_data"`
}
